/**
 * Poseidon2 Hash Function for BN254
 *
 * This implementation matches Noir's stdlib poseidon2 hash function exactly.
 * Parameters extracted from: noir-lang/noir/acvm-repo/bn254_blackbox_solver/src/poseidon2_constants.rs
 *
 * Configuration:
 * - State size (t): 4
 * - S-box power (d): 5 (x^5)
...
// Number of rounds
const ROUNDS_F = 8; // Full rounds
const ROUNDS_P = 56; // Partial rounds

// Internal matrix diagonal (for linear layer in partial rounds)
const INTERNAL_MATRIX_DIAGONAL: bigint[] = [
...
// Round constants for all 64 rounds (4 elements each)
// First 4 rounds are full rounds (r_f/2 = 4)
// Next 56 rounds are partial rounds (only first element is non-zero)
// Last 4 rounds are full rounds (r_f/2 = 4)
const ROUND_CONSTANTS: bigint[][] = [
  // Rounds 0-3: Full rounds (beginning)
  [
  ...
  // Rounds 4-59: Partial rounds (only first element non-zero)
  ...
  // Rounds 60-63: Full rounds (ending)
  [
...
function mod(x: bigint): bigint {
  const result = x % BN254_MODULUS;
  return result >= 0n ? result : result + BN254_MODULUS;
}
...
/**
 * External matrix multiplication (for full rounds)
 * This is the exact algorithm from Noir's poseidon2.rs:matrix_multiplication_4x4
 *
 * The matrix produces:
 *   output[0] = 5A + 7B + 1C + 3D
 *   output[1] = 4A + 6B + 1C + 1D
 *   output[2] = 1A + 3B + 5C + 7D
 *   output[3] = 1A + 1B + 4C + 6D
 */
 ...
 * From Noir's poseidon2.rs:internal_m_multiplication
 *
 * Algorithm:
 *   sum = sum(state)
 *   for each i: state[i] = state[i] * diagonal[i] + sum
 */
function matmulInternal(state: bigint[]): bigint[] {
  // Sum all elements first
  const sum = mod(state[0] + state[1] + state[2] + state[3]);

  // Apply: result[i] = state[i] * diagonal[i] + sum
  return [
    addMod(mulMod(state[0], INTERNAL_MATRIX_DIAGONAL[0]), sum),
    addMod(mulMod(state[1], INTERNAL_MATRIX_DIAGONAL[1]), sum),
    addMod(mulMod(state[2], INTERNAL_MATRIX_DIAGONAL[2]), sum),
    addMod(mulMod(state[3], INTERNAL_MATRIX_DIAGONAL[3]), sum),
  ];
}

/**
 * Full round: AddRC + S-box (all elements) + Linear layer
 */
function fullRound(state: bigint[], roundIndex: number): bigint[] {
  const rc = ROUND_CONSTANTS[roundIndex];
  ...
  newState = [sbox(newState[0]), sbox(newState[1]), sbox(newState[2]), sbox(newState[3])];

  // External matrix multiplication
  return matmulExternal(newState);
}

/**
 * Partial round: AddRC (first element only) + S-box (first element only) + Linear layer
 */
function partialRound(state: bigint[], roundIndex: number): bigint[] {
  const rc = ROUND_CONSTANTS[roundIndex];

  // Add round constant (only to first element)
  const newState = [addMod(state[0], rc[0]), state[1], state[2], state[3]];

  // S-box only on first element
  newState[0] = sbox(newState[0]);

  // Internal matrix multiplication
  return matmulInternal(newState);
}

/**
 * Poseidon2 permutation
 *
 * Structure (from Noir's poseidon2.rs):
 * 1. Initial linear layer (matrix_multiplication_4x4)
 * 2. First 4 full rounds
 * 3. 56 partial rounds
 * 4. Last 4 full rounds
 */
export function poseidon2Permutation(input: bigint[]): bigint[] {
  if (input.length !== T) {
    throw new Error(`Poseidon2 requires exactly ${T} inputs, got ${input.length}`);
  }

  let state = [...input];

  // Apply initial linear layer BEFORE rounds
 ...
 *
 * This matches Noir's `std::hash::poseidon2::Poseidon2::hash()` for 2 inputs.
 */
export function poseidon2Hash(left: bigint, right: bigint): bigint {
  // Initialize state: [left, right, 0, 0]
...
/**
 * Convert a field element to a 32-byte hex string (with 0x prefix)
 */
...
/**
 * Pre-computed empty subtree roots for a 20-level SMT
 *
 * EMPTY_SUBTREE_ROOTS[i] = hash of an empty subtree at level i
 * Level 0 = leaf level (empty leaf = 0)
 * Level 20 = root level
 */
export function computeEmptySubtreeRoots(depth: number): bigint[] {
  const roots: bigint[] = [0n]; // Level 0: empty leaf = 0
...
// Pre-computed empty subtree roots for the 20-level blacklist SMT
export const POSEIDON2_EMPTY_SUBTREE_ROOTS = computeEmptySubtreeRoots(20);

// The empty tree root (level 20)
export const POSEIDON2_EMPTY_TREE_ROOT = POSEIDON2_EMPTY_SUBTREE_ROOTS[20];
