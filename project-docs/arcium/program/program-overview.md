# Invoking Encrypted Instructions from Solana Programs

Before reading this, we recommend having read the [Computation Lifecycle](../computation-lifecycle.md) section, the [Arcis inputs/outputs](../arcis/input-output.md) section, and the [Callback Type Generation](callback-type-generation.md) guide to understand how output types like `AddTogetherOutput` are automatically generated from encrypted instructions.
...
But what if we don't just want to return a raw value and need some additional accounts? Check out [input/outputs](../arcis/input-output.md) for how to handle encrypted data and [callback accounts](callback-accs.md) for returning additional accounts in the callback.
