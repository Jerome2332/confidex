...
Encrypted data can be decrypted globally or selectively to a given user. For global decryption, you can call `reveal` method on any variable of [supported data type](../arcis/types.md). Read more about how we enable this using re-encryption (aka sealing) in Arcium in [sealing.md](sealing.md).
...
1. Since the x25519 key exchange natively returns shared secrets in the finite field with p = 2^255 - 19 elements, we implemented Rescue over the field 𝔽_p. States in the context of Rescue are elements of the m-dimensional vector space 𝔽_p^m, i.e., the Rescue cipher transforms vectors of size m to vectors of the same size.
